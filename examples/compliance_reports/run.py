...
PROFILES: list[dict] = [
    {
        ...
        "label": f"General AI System — All {len(ALL_FRAMEWORK_IDS)} Frameworks",
        "description": (
...
def _build_index(
    ...
    lines = [
        ...
        f"against up to {len(ALL_FRAMEWORK_IDS)} regulatory frameworks using acgs-lite's "
        ...
        "| **ACGS coverage** | Fraction of requirements for which acgs-lite provides "
    ...
    lines += [
        "---",
        "",
        f"## All {len(ALL_FRAMEWORK_IDS)} Supported Frameworks",
        "",
...
This shows what fraction of each framework's requirements acgs-lite provides
supporting technical measures for — without any configuration beyond a
`Constitution` and `AuditLog`.
